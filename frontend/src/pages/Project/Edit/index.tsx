...
export function EditProject() {
  ...
  const { data: projectData, isLoading: isProjectLoading } = useQuery({
    queryKey: ["project", titulo],
    queryFn: () => getProjectByTitle(titulo!),
    enabled: !!titulo,
  });
  ...
  useEffect(() => {
    if (projectData) {
      reset({
        projectName: projectData.titulo,
        projectDescription: projectData.descricao,
        projectInitialDate: projectData.dataInicio,
  ...
  const mutation = useMutation({
    mutationFn: (data: z.infer<typeof schema>) => {
      const payload = {
        ...
        dataInicio: data.projectInitialDate,
        status: data.projectStatus,
        gerente: data.gerente,
      };
      return updateProjectByTitle(titulo!, payload);
    },
    onSuccess: () => {
      ...
      >
        <div>
          ...
          <Input
            ...
            className="w-full"
            disabled // O nome do projeto (título) não pode ser editado
          />
          {errors.projectName && (
